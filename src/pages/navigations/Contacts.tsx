import { ArrowLeftIcon, SearchIcon, UserAddIcon } from "tdesign-icons-react";
...
export default function Contacts({ navGoBack }: { navGoBack: Function }) {
    ...
    const [hasNewReq, setHasNewReq] = useState(true);
    ...
    return <div className="gap-1.5 max-h-full flex flex-col">
        ...
        <div className="flex relative gap-0.5">
            ...
            <div className="border-button h-full p-1.5 flex gap-1 scale-90 items-center justify-items-center relative">
                ...
                <div>New</div>
                {
                    hasNewReq && <i className="bg-red-400 rounded-full shadow absolute block z-20 -top-1.5 -right-1.5 size-3"></i>
                }
            </div>
        </div>
        <div className="relative mb-3">
            ...
            <Input placeholder={searching ? 'Search in contacts...' : ''} onFocus={() => setSearching(true)} onBlur={() => setSearching(false)} className={`w-full block-shadow h-8 rounded-lg px-1 interactive outline-0`} />
        </div>
        <NavList items={mock} hintBottom />
    </div>
}